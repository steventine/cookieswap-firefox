...
function cookieswap_mgrLoad()
{
  ...
  gCsProfileMgr = getCookieSwapProfileMgrModule(false);
  ...
  curr_checkbox = document.getElementById('cs-pref-display-name-in-toolbar');
  curr_checkbox.checked = gCsPrefsOptions.getBoolPref("profileUI.DisplayProfileNameInToolbar");
...
function cookieswap_mgrClose()
{
  ...
  curr_checkbox = document.getElementById('cs-pref-display-name-in-toolbar');
  gCsPrefsOptions.setBoolPref("profileUI.DisplayProfileNameInToolbar", curr_checkbox.checked);
...
//Called when the ManageProfile window's COPY button is clicked
function cookieswap_mgrCopy()
{
  cookieswap_dbg("enter cookieswap_mgrCopy()");
  ...
  {
     var profile_name = list_item.getAttribute("profile_name");
     cookieswap_dbg("mgrCopy...profile_name:" + profile_name + " value:" + list_item.value + 
           " MyInfo:" + list_item.getAttribute("MyInfo"));
     var reply = prompt("Enter name for new profile:", profile_name);
     if (reply != null && reply != profile_name)
     {
        if (cookieswap_mgrDoesProfileExist(reply))
        {
        ...
        {
           //Ready to copy the profile.  Don't perform checking here to determine
           //  if the name is valid for the file system.  Just catch that exception.
           cookieswap_dbg("copyProfile(" + profile_name + "," + reply + ")");
           try
           {
              //Request the profile copy
              //Do this via the cloneProfile...put the profile name first, then the new name with
              //  a comma inbetween.
              gCsProfileMgr.cloneProfile(profile_name + " , " + reply );
           }
           catch (err) 
           { 
              //Rename through exception.  Most likely because the chars provided
              //  are not valid for a filename
              cookieswap_dbg("copyProfile exception:" + err); 
              alert("Copy failed.  CookieSwap profiles must only use characters that are valid in file names."); 
           }
   
           cookieswap_dbg("item copied");
        }
     }
     else
     {
  ...
  {
     //This shouldn't happen
     alert("Please select a profile to copy");
  }
}
...
function cookieswap_mgrSelect(listboxSelectedItem)
{
  ...
  if(listboxSelectedItem != null)
  {
	...
	{
	   //Yes, don't allow remove/copy of the active profile
...
cookieswap_ProfileMgrObserver.prototype = {
  observe: function(subject, topic, data) {
   ...
   cookieswap_dbg("ProfileMgrObserver called! " + subject + "(" + (subj.cookieSwapProfileMgr == gCsProfileMgr) + "):" + data);
   
   //First check to see if this notifiation is from the public ProfileMgr (the only one managed by this class)
