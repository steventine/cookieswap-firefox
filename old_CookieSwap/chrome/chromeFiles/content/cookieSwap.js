...
//This tracks if the current window is in private browsing mode or not
var gIsWindowPrivate=false;

//Defines that are part of the interface definiton with the ProfileMgr Service
const INVALID_PROFILE_ID=-1;

Components.utils.import("resource://cookieswap_res/CookieSwapProfileMgrModule.js");
...
function cookieswap_init(event)
{
   ...
   //Get the private browsing state of this window
   gIsWindowPrivate = PrivateBrowsingUtils.isWindowPrivate(window);

   //Check if this is the first time CookieSwap has run after adding support
   //  for the toolbar button.
   var firstToolbarRunPref = Components.classes['@mozilla.org/preferences-service;1']
                 .getService(Components.interfaces.nsIPrefService);
   firstToolbarRunPref = firstToolbarRunPref.getBranch('extensions.cookieswap.');
   firstToolbarRun = firstToolbarRunPref.getBoolPref('firstToobarRun'); 
   if (firstToolbarRun == true)  
   {
      //First time CookieSwap has run since the Toolbar button was added.
      //  Install the toolbar button and set the flag to false so
      //  we don't install the button again in case the user has removed it.
      firstToolbarRunPref.setBoolPref('firstToobarRun', false); 
      installButton("nav-bar", "cookieswap-toolbar-button");  

      //Also bring up the new Toolbar Help page.
      //  I generally don't like to do this but I think it is
      //  justified as this was a major change.
      cookieswap_loadToolbarHelpTab();
   }
   ...
   gCsProfileMgr = getCookieSwapProfileMgrModule(gIsWindowPrivate);
   cookieswap_dbg("Created ProfileMgr Service...isWindowPrivate=" + gIsWindowPrivate);

   //Check to see if there are any private browsing windows open
   if((gIsWindowPrivate == false) && anyOpenPrivateBrowsingWindows())
   {
       ...
       {
           cookieswap_dbg("Forcing window into private mode");
           gIsWindowPrivate = true;
       }   
   }
...
function cookieswap_populateProfilesToUI(profile_UI)
{
   ...
   if (gIsWindowPrivate == true)
   {
       //If priv browsing, set the profile_UI
       profile_UI.enterPrivateBrowsing();
   }
}
...
function cookieswap_loadHelpTab()
{
  ...
  cookieswap_openLinkInBrowserTab('http://cookieswap.tinefamily.com/help');
}

function cookieswap_loadToolbarHelpTab()
{
  //Load the Help page on a new tab
  //gBrowser.addTab('http://cookieswap.mozdev.org/help.html');
  cookieswap_openLinkInBrowserTab('http://cookieswap.tinefamily.com/help/cookieswap-toolbar');
}

function cookieswap_getVersion()
{
  return(getCookieSwapVersion());
}
...
function cookieswap_unitTest()
{
   ...
   //--cloneProfile
   gCsProfileMgr.cloneProfile("test");
   cookieswap_dbg("Cloned Profile");
       
   //--profileCount
   cookieswap_dbg("Profile Count = " + gCsProfileMgr.profileCount);
 
   //--createNewProfile
   gCsProfileMgr.createNewProfile("test1", "test1dir", "", true);
 
   //--deleteProfile
   gCsProfileMgr.deleteProfile("test1", true);
   gCsProfileMgr.deleteProfile("test1", false);
   ...
   //--profileExists
   cookieswap_dbg("test exists = " + gCsProfileMgr.profileExists("test"));
   cookieswap_dbg("test1 exists = " + gCsProfileMgr.profileExists("test1"));
   
   //renameProfile
   gCsProfileMgr.renameProfile("test", "test2");
   
   //shutDownCurrentProfile
   gCsProfileMgr.shutDownCurrentProfile(Components.interfaces.nsIProfile.SHUTDOWN_PERSIST);
   gCsProfileMgr.shutDownCurrentProfile(Components.interfaces.nsIProfile.SHUTDOWN_CLEANSE);
   
   cookieswap_dbg("currentProfile = " + gCsProfileMgr.currentProfile);
   gCsProfileMgr.currentProfile = "test1";
   cookieswap_dbg("currentProfile = " + gCsProfileMgr.currentProfile);
   gCsProfileMgr.currentProfile = "test2";
   cookieswap_dbg("currentProfile = " + gCsProfileMgr.currentProfile);
}
...
cookieswap_Observer.prototype = {
  observe: function(subject, topic, data) {
   var subj = subject.wrappedJSObject;
   
   cookieswap_dbg("Observer called! " + subject + "(" + (subj.cookieSwapProfileMgr == gCsProfileMgr) + "):" + data);
   
   //First check to see if this notifiation is from our ProfileMgr
   if (subj.cookieSwapProfileMgr == gCsProfileMgr)
   {
...
{
   //Leavin private browsing mode
   var profile_UI = profileUI_getInstance();
   profile_UI.exitPrivateBrowsing();
   gIsWindowPrivate = false;
}
...
function cookieswap_dtor(event)
{
   ...
   //Release the ProfileManager
   releaseCookieSwapProfileMgrModule(gCsProfileMgr);
   gCsProfileMgr = null;

   //Destroy the profileUI singleton
   profileUI_destroyInstance()
